		...
		try{
			...
			foreach($stmt as $row){
				...
				if($row['dayss']>1){
					$sum = 0;
					$charge = 0.10;
                for ( $i = 1; $i <=$row['dayss']; $i++){
					$sum  += $charge ;
                    
                            }	
							$daycharge = $carttotal * $sum;
				...
				$output .= "
					<tr>
						...
						<td class='input-group'>
							<span class='input-group-btn'>
            					<button type='button' id='daysminus' class='btn btn-default btn-flat minus' data-id='".$row['cartid']."'><i class='fa fa-minus'></i></button>
            				</span>
            				<input type='text' class='form-control' value='".$row['dayss']."' id='days_".$row['cartid']."' min='1' max = '10' steps= '3'>
							<span class='input-group-btn'>
				                <button type='button' id='daysadd' class='btn btn-default btn-flat add' data-id='".$row['cartid']."'><i class='fa fa-plus'></i>
				                </button>
				            </span>
						</td>
										</tr>
