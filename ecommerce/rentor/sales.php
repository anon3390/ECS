                ...
                <tbody>
                  <?php
                    ...
                    try{
                      ...
                      foreach($stmt as $row){
                        $stmt = $conn->prepare("SELECT * FROM details LEFT JOIN products ON products.id=details.product_id WHERE details.sales_id=:id AND products.rentor_id =:rent_id");
                        $stmt->execute(['id'=>$row['salesid'],'rent_id'=>$rentor['id']]);
                        // try{
                //           $stmt = $conn->prepare("SELECT * FROM details LEFT JOIN products ON products.id=details.product_id where products.rentor_id =:rent_id");
                // $stmt->execute(['rent_id'=>$rentor['id']]);
                        $total = 0;
